...
/**
 * This class encapsulates all functionality for drawing the current view at the
 * maze from a first person perspective. It is an drawing agent with redraw as
 * its public method to redraw the maze while the user plays, i.e. navigates
 * through the maze.
 *
...
public class FirstPersonDrawer extends DefaultViewer {
	// keep local copies of values determined in MazeController.java, all values are
	// set in the constructor call
	// values are basically constants or shared data structures across
	// MazeController, MapDrawer and FirstPersonDrawer
	// constants, i.e. set in constructor call with values that are not subject to
	// change in Maze
	...
	// node is determined in MazeBuilder when creating the maze, helps to decide
	// visibility
	...
	// the graphics object for the buffer image this class draws on
	// note: updating the panel that is on screen with the buffer image is the
	// responsibility of the maze controller class

	final int viewz = 50; // constant from MazeController.java

	// current position (px,py) scaled by map_unit and modified by view direction is
	// stored in (viewx, viewy)
	 ...
	 * @param bsp_root
	 */
	public FirstPersonDrawer(int width, int height, int map_unit, int step_size, Cells seencells, BSPNode bsp_root, MazePanel p) {
		...
		angle = 0; // angle for initial setting of direction is 0 == East, hidden constraint across
					// classes
	...
	// for increment/decrementmapscale: use dummy implementation inherited from
	///////////////// defaultviewer
	//
	/**
	 ...
	 * @param px
	 *            x coordinate of current position, only used to get viewx
	 * @param py
	 *            y coordinate of current position, only used to get viewy
	 * @param view_dx
	 *            view direction, x coordinate
	 * @param view_dy
	 *            view direction, y coordinate
	 * @param rset
	 * @param ang
	 * @param walk_step,
	 *            only used to get viewx and viewy
	 * @param view_offset,
	 *            only used to get viewx and viewy
	 */
	@Override
	public void redraw(MazePanel panel, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset,
	 ...
	 * @param gc
	 */
	private void drawBackground(MazePanel p) {
		p.setMazeColor("black");
		p.fillRect(0, 0, view_width, view_height / 2);
		p.setMazeColor("darkGray");
		p.fillRect(0, view_height / 2, view_width, view_height / 2);
	}

	/**
	 * Recursive method to explore tree of BSP nodes and draw all segments in leaf
	 * nodes where the bounding box is visible
	 * 
	 * @param nn
	 *            is the current node of interest
	 */
	private void drawAllVisibleSectors(BSPNode nn) {
		...
		// debug code
		if (deepdebug) {
			dbg("                               ".substring(0, nesting) + "traverse_node " + n.getX() + " " + n.getY()
					+ " " + n.getDx() + " " + n.getDy() + " " + n.getLowerBoundX() + " " + n.getLowerBoundY() + " "
					+ n.getUpperBoundX() + " " + n.getUpperBoundY());
		}
		nesting++; // debug
	...
	private boolean boundingBoxIsVisible(BSPNode b) {
		...
		// constraint: x1 <= x2
		return (panel.intersect(rset, new Coordinates(x2, x1)));
	}

	/**
	 * Traverses all segments of this leaf and draws corresponding rectangles on
	 * screen
	 * 
	 * @param n
	 *            is the leaf node
	...
	private void drawAllSegmentsOfASector(BSPLeaf n) {
		...
		traverse_ssector_ct++;
		if (deepdebug) {
			dbg("                               ".substring(0, nesting) + "traverse_ssector " + n.getLowerBoundX() + " "
					+ n.getLowerBoundY() + " " + n.getUpperBoundX() + " " + n.getUpperBoundY());
		}
		// for all segments of this node
		for (int i = 0; i != sl.size(); i++) {
			Seg seg = (Seg) sl.get(i);
			// draw rectangle
			// before: drawSegment(seg, seg.getStartPositionX(), seg.getStartPositionY(),
			// seg.getEndPositionX(), seg.getEndPositionY());
			drawSegment(seg);
			// debug
			if (deepdebug) {
				dbg("                               ".substring(0, nesting) + " traverse_ssector(" + i + ") "
						+ seg.getStartPositionX() + " " + seg.getStartPositionY() + " " + seg.getExtensionX() + " "
						+ seg.getExtensionY());
			}
	...
	/**
	 * Draws segment on screen via graphics attribute gc Helper method for
	 * traverse_ssector
	 * 
	 * @param seg
	 *            whose seen attribute may be set to true
	 */
	private void drawSegment(Seg seg) {
		...
		while (x1i <= x2) {
			...
			Coordinates point = new Coordinates(x2, x1i);
			
			if (!panel.intersect(rset, point))
				break;
			x1i = point.x;
			int x2i = point.y;
			// let's work on the intersection (x1i,x2i)
			int[] xps = { x1i, x1i, x2i + 1, x2i + 1 };
			int[] yps = { y11 + (x1i - x1) * (y21 - y11) / xd, y12 + (x1i - x1) * (y22 - y12) / xd + 1,
					y22 + (x2i - x2) * (y22 - y12) / xd + 1, y21 + (x2i - x2) * (y21 - y11) / xd };
			// debug
			// System.out.println("polygon-x: " + xps[0] + ", " + xps[1] + ", " + xps[2] +
			// ", " + xps[3]) ;
			// System.out.println("polygon-y: " + yps[0] + ", " + yps[1] + ", " + yps[2] +
			// ", " + yps[3]) ;
			...
			/*
			 * gc.setColor(new Color(240,20,20)); gc.drawPolygon(xps, yps, 4);
	 ...
	 * 
	 * @param seg
	 *            segment (only read access)
	 */
	private void udpateSeenCellsForSegment(Seg seg) {
		/*
		 * original code // TODO: move method to Cells.java class:
		 * seencells.addSegment(Seg seg) // Why: changes seencells by adding walls based
		 * on info in given seg // only piece of information used from FirstPersonDrawer
		 * is map_unit
		 * 
		 * // we need to obtain the starting position (sx,sy) of the segment // and the
		 * direction (sdsx,sdsy) in which the segment proceeds // Step 1: get the
		 * direction of the segment final int sdx = seg.getExtensionX() / map_unit; //
		 * constant, only set once here final int sdy = seg.getExtensionY() / map_unit;
		 * // constant, only set once here
		 * 
		 * // Step 2: get initial position (sx,sy) right int sx =
		 * seg.getStartPositionX() / map_unit; if (sdx < 0) // this direction and
		 * negative sx--; int sy = seg.getStartPositionY() / map_unit; if (sdy < 0) //
		 * this direction and negative sy--;
		 * 
		 * // define constants to avoid method calls in following loop final int sdsx =
		 * MazeBuilder.getSign(sdx); final int sdsy = MazeBuilder.getSign(sdy);
		 * CardinalDirection cd = (sdx != 0) ? CardinalDirection.North :
		 * CardinalDirection.West ; //final int len = Math.abs(sdx + sdy); //if (len !=
		 * seg.length() / map_unit) { // System.out.print("Error: seg length wrong: " +
		 * len + " vs " + seg.length()); //} final int len = seg.getLength() / map_unit;
		 * Wall wall = new Wall(0,0,CardinalDirection.East) ; // values don't matter //
		 * true loop variables are (sx,sy), a position in the maze for (int i = 0; i !=
		 * len; i++) { // cd is either NORTH or WEST // so we basically add a wall NORTH
		 * or WEST // but only from one side for the given cell (not its neighbor)
		 * wall.setWall(sx, sy, cd); seencells.addWall(wall, false) ; // move to
		 * neighbor cell in the direction of the segment sx += sdsx; sy += sdsy; }
		 */
		// new code after relocation, inline if stable
		seencells.addWallsForSegment(seg, map_unit);
	}

	////////////////////////////// static methods that do not rely on instance
	////////////////////////////// fields //////
	/**
	 * Unscale given vale
	 * 
	 ...
	 * @param num
	 * @param fp
	 *            is a parameter whose internal attributes p1 and p2 may be modified
	 * @return
	 */
	static private boolean clipt(int denom, int num, FloatPair fp) {
		...
		/*
		 * if (denom > 0) { double t = num * 1.0 / denom; if (t > fp.p2) return false;
		 * if (t > fp.p1) fp.p1 = t; // update fp } else if (denom < 0) { double t = num
		 * * 1.0 / denom; if (t < fp.p1) return false; if (t < fp.p2) fp.p2 = t; //
		 * update fp } else if (num > 0) return false; return true;
		 */
		// new code after relocation, inline if stable
	...
	protected boolean clip3d(RangePair rp) {
		 ...
		 * 
		 * if (z1 > -4 && z2 > -4) return false; if (x1 > -z1 && x2 > -z2) return false;
		 * if (-x1 > -z1 && -x2 > -z2) return false; final int dx = x2-x1; final int dz
		 * = z2-z1; FloatPair fp = new FloatPair(0, 1); if (!clipt(-dx-dz, x1+z1, fp))
		 * return false; if (!clipt( dx-dz,-x1+z1, fp)) return false; if (!clipt(-dz,
		 * z1-4, fp)) return false; // update internals of parameter rp if (fp.p2 < 1) {
		 * rp.x2 = (int) (x1 + fp.p2*dx); rp.z2 = (int) (z1 + fp.p2*dz); } if (fp.p1 >
		 * 0) { rp.x1 += fp.p1*dx; rp.z1 += fp.p1*dz; } return true;
		 */
		// new code after relocation, inline if stable
		return rp.clip3d();
	}
