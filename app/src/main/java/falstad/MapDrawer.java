 ...
 * the set of visible walls, the solution. The map is drawn on the screen in
 * such a way that the current position remains at the center of the screen. The
 * current position is visualized as a red dot with an attached arc for its
 * current direction. The solution is visualized as a yellow line from the
 * current position towards the exit of the map. Walls that are currently
 * visible in the first person view are drawn white, all other walls are drawn
 * in grey. It is possible to zoom in and out of the map.
 * 
...
public class MapDrawer extends DefaultViewer {
	...
	/**
	 * Seencells contains information on walls that are seen from the current point
	 * of view. The field is set by the constructor. The referenced object is shared
	 * with the FirstPersonDrawer that writes content into it. The MapDrawer only
	 ...
	 * @param c
	 */
	public MapDrawer(int width, int height, int map_unit, int step_size, Cells seencells, int map_scale,
			MazeController c) {
	...
	public void redraw(MazePanel panel, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset,
		...
		if (null != controller) {
			if (controller.isInMapMode()) {
				draw_map(px, py, walk_step, view_dx, view_dy, controller.isInShowMazeMode(),
	...
	//////////////////////////////// //////////////////////////////
	/**
	 * Helper method for redraw, called if map_mode is true, i.e. the users wants to
	 * see the overall map. The map is drawn only on a small rectangle inside the
	 * maze area such that only a part of the map is actually shown. Of course a
	 * part covering the current location needs to be displayed.
	 * 
	 * @param gc
	 *            graphics handler to manipulate screen
	 */
	public void draw_map(int px, int py, int walk_step, int view_dx, int view_dy, boolean showMaze,
		...
		for (int y = ymin; y <= ymax; y++)
			for (int x = xmin; x <= xmax; x++) {
				...
				panel.setMazeColor(seencells.hasWall(x, y, CardinalDirection.North) ? "white" : "gray");
				...
				panel.setMazeColor(seencells.hasWall(x, y, CardinalDirection.West) ? "white" : "gray");
	...
	/**
	 * Draws an oval red shape with and arrow for the current position and direction
	 * on the maze. It always reside on the center of the screen. The map drawing
	 * moves if the user changes location.
	 * 
	 * @param gc
	 *            to draw on
	 */
	...
	/**
	 * Draws a yellow line to show the solution on the overall map. Method is only
	 * called if in STATE_PLAY and map_mode and showSolution are true. Since the
	 * current position is fixed at the center of the screen, all lines on the map
	 * are drawn with some offset.
	 * 
	 * @param gc
	 *            to draw lines on
	 * @param offx
